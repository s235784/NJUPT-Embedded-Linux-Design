...
以下以直接在ARM Linux下编译为例：

首先clone本仓库至ARM Linux，并安装依赖，然后依次运行
...
编译完成后把build-linux目录下的`program_design`文件上传至试验箱的Linux内，运行
...
征询老师后直接在Linux下读取传感器数据，并使用WebSocket传输至Web端。
...
使用Zigbee与传感器相连接。需要注意的是，在Linux的Zigbee软件中设置无线通道和PANID后，单片机内需要保证这两个参数一致；使用完后需要重启整个机器。
...
使用蓝牙读取传感器数据。需要注意的是读取传感器数据时必须要等待一定的时间，不然会卡住（本项目已在代码中设计了延迟）。

另外，发现无法建立连接时需要reset单片机和Linux的蓝牙模块。
